...
# --- Configuración de la Página e Inicialización ---
st.set_page_config(page_title="Perfil - MedCheck", page_icon="👤", layout="centered")
...
conn = connect_to_supabase() # Se necesita para la verificación de la encuesta
...
st.write("Bienvenido a tu perfil médico. Genera informes para tus consultas o emergencias.")
st.divider()

# --- 4. Sección de Generación de Informes ---
st.header("📄 Generar Informe Médico")
tipo_informe = st.selectbox("Selecciona el tipo de informe", ["guardia_agil", "historial_completo", "personalizado"], format_func=lambda x: {"guardia_agil": "🚑 Guardia Ágil", "historial_completo": "📋 Historial Completo", "personalizado": "⚙️ Personalizado"}[x], label_visibility="collapsed")

# --- 5. Opciones de Informe ---
opciones_informe = { "incluir_alergias": True, "incluir_medicacion": True, "incluir_antecedentes": True, "incluir_vacunas": True, "incluir_estudios": True, "incluir_contacto": True }
if tipo_informe == "guardia_agil":
    opciones_informe.update({"incluir_antecedentes": False, "incluir_vacunas": False, "incluir_estudios": False})
# ... (lógica de opciones sin cambios)
...
with col2:
    ...
    st.download_button(
        ...
        disabled=is_disabled or not pdf_bytes,
        type="primary"
    )
